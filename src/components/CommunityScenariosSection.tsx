    ...
    useEffect(() => {
        const fetchScenarios = async () => {
            ...
            try {
                ...
                console.error("🔥 COMMUNITY FEED ERROR:", err.code, err.message);
                if (err.message?.includes("index")) {
                    console.log("👉 Suggestion: Create a Collection Group Index for 'scenarios' with 'isPublic' (ASC) and 'createdAt' (DESC).");
                }
                setScenarios([]);
            } finally {
                setLoading(false);
